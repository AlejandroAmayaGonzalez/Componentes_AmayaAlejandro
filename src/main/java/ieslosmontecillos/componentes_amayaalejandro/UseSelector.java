...
public class UseSelector extends VBox {
    ...
    // Constructor
    public UseSelector() {
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("UseSelector.fxml"));
    ...
    private void updateItem() {
        if (items.isEmpty()) {
            ...
            if (selectedIndex >= items.size()) {
                if (repetitive) {selectedIndex = 0;
                } else {
                    selectedIndex = items.size() - 1;
                }
            }
            lb.setText(items.get(selectedIndex));
        }
    }
